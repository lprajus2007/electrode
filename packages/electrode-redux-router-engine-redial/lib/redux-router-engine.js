...
class ReduxRouterEngine {
  constructor(options) {
    ...
    this.options.enableRedial = !!options.enableRedial;
  ...
  _handleRender(req, match, route, options) {
    ...
    const enableRedial =
      options.enableRedial || this.options.enableRedial

    return this._getReduxStoreInitializer(route, options).call(this, req, match)
      .then((store) => {
        const r = {};
        const x = this._renderToString(req, store, match, {enableRedial, withIds});
  ...
  _renderToString(req, store, match, { enableRedial, withIds }) { // eslint-disable-line
    if (req.app && req.app.disableSSR) {
      return "";
    } else if (enableRedial) {
      const {dispatch} = store
      const locals = {
        path: match.renderProps.location.pathname,
        query: match.renderProps.location.query,
        params: match.renderProps.params,
        dispatch
      }
      return trigger('fetch', match.renderProps.components, locals)
    } else {
